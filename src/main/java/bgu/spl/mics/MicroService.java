...
public abstract class MicroService implements Runnable {
    ...
    public final void run() {
        ...
        initialize();
        while (!terminated){
            Message currentTask = null;
            try {
                currentTask = mb.awaitMessage(this);
            }
            catch (InterruptedException e){}
//            if(currentTask.toString().startsWith("bgu.spl.mics.application.messages.Train"))
//                System.out.println(((TrainModelEvent)currentTask).getModel().getGpu().toString()+"  recieved:  " +((TrainModelEvent)currentTask).getModel().getName());
//            if(currentTask.toString().startsWith("bgu.spl.mics.application.messages.Test"))
//                System.out.println(((TestModelEvent)currentTask).getModel().getName());
            callbacks.get(currentTask.getClass()).call(currentTask);
        }
        mb.unregister(this);
    }
}